...
【回答ルール】
1. 200字以内で簡潔にまとめる（LINEで読みやすい長さ）
2. 専門用語を避け、誰でも分かる言葉で説明する
3. 手続きの窓口・持ち物・締め切りがある場合は明示する
4. 具体的な内容が分からない場合は「詳細は担当窓口にお問い合わせください」と案内する
5. 個人情報や具体的な申請内容の判断には言及しない
6. 前置きや「回答します」などの定型文は不要。すぐに本題を答える

${context ? `【参考情報】\n${context}\n` : ''}
上記の参考情報がある場合は活用してください。ない場合は一般的な行政知識で回答してください。`

    const res = await anthropic.messages.create({
      model:      modelUsed,
      max_tokens: 300,  // 200字 ≒ 200〜250トークン
      system:     systemPrompt,
    ...
    // 文字数制限チェック（200字超の場合は省略）
    const truncated = answerRaw.length > 200
