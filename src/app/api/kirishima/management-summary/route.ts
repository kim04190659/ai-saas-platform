// =====================================================
//  src/app/api/kirishima/management-summary/route.ts
//  霧島市 経営ダッシュボード 集約API — Sprint #53
//
//  ■ エンドポイント
//    GET /api/kirishima/management-summary
//
//  ■ 概要
//    財政健全化・インフラ老朽化・PDCA進捗・住民WBの4領域から
//    KPIサマリーを並列取得してまとめて返す。
//    各エンジンを直接呼び出し（内部HTTPリクエストなし）。
//
//  ■ レスポンス構造
//    { fiscal{}, infra{}, pdca{}, wb{}, updatedAt }
// =====================================================
...
export async function GET() {
  ...
  try {
    ...
    const [fiscalData, infraData, pdcaData, wbData] = await Promise.allSettled([
      fetchFiscalIndicators(notionKey, MUNICIPALITY_ID),
      fetchInfraFacilities(notionKey, MUNICIPALITY_ID),
      fetchPolicies(notionKey, MUNICIPALITY_ID),
    ...
    console.error('[management-summary] エラー:', e)
    return NextResponse.json({
      status: 'error',
