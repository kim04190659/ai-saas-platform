...
// =====================================================
//  src/app/(dashboard)/gyosei/line-consultation/page.tsx
//  LINE相談ログ 職員対応管理UI — Sprint #15
//
//  ■ このページの役割
//    住民からLINEで届いた相談を職員が一覧確認し、
//    対応状況（未対応→対応中→完了）を画面から更新できる。
//    Notionのデータベースと双方向で連携する。
//
//  ■ 機能概要
...
//    - GET /api/line-consultation で一覧取得
//    - PATCH /api/line-consultation で対応状況を更新
// =====================================================
...
/** 担当部署の選択肢 */
const DEPARTMENT_OPTIONS = ['住民課', '総務課', '福祉課', '財政課', '情報政策課', '教育委員会', '観光課']
        ...
        <div className="px-4 pb-4 space-y-3 border-t border-slate-100 pt-3">
          ...
          {record.aiResult && (
            <div className="bg-violet-50 rounded-lg p-3">
              <p className="text-xs font-medium text-violet-600 mb-1">🤖 AI振り分け結果</p>
              <p className="text-sm text-violet-700">{record.aiResult}</p>
            </div>
          )}
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-4 py-2 rounded-lg bg-sky-600 text-white text-sm font-medium hover:bg-sky-700 transition-colors disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed"
            >
...
export default function LineConsultationPage() {
  ...
  const [activeFilter, setActiveFilter] = useState<string>('')   // 現在のフィルタータブ
      ...
      <div className="max-w-4xl mx-auto space-y-5">
        ...
        <div className="bg-sky-50 rounded-2xl border border-sky-200 p-4">
          ...
          <p className="text-sm text-sky-700 leading-relaxed">
            このページは<strong>LINE相談ログDBのデータ</strong>をもとに表示されています。
            実際の住民からのLINE相談はNotionフォームで受け付け、
            職員はこの画面で対応状況を管理します。
          ...
          <div className="mt-3 flex gap-2 flex-wrap">
            <Link
              href="/ai-advisor"
              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-sky-100 text-sky-700 hover:bg-sky-200 transition-colors border border-sky-200"
            >
              🤖 AI顧問で相談トレンドを分析
            </Link>
            <Link
              href="/gyosei/dashboard"
              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-violet-50 text-violet-700 hover:bg-violet-100 transition-colors border border-violet-200"
            >
              📊 Well-Beingダッシュボードへ
            </Link>
          </div>
