'use client'
// =====================================================
//  src/components/yakushima/YakushimaPdcaPanel.tsx
//  屋久島町 PDCA 追跡パネル — Sprint #47
//
//  ■ 機能
//    1. カンバン：検討中 → 実施中 → 完了 の施策一覧
//    2. 効果測定：完了施策の Before/After を表示
//    3. AI 総括：Claude が PDCA サイクルを評価し次のアクションを提言
//    4. 施策登録フォーム：Sprint #46 提案をワンクリック登録
// =====================================================
...
function RegisterForm({ onRegister }: RegisterFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true)
    await fetch('/api/yakushima/pdca-tracking', {
      method: 'POST',
    ...
    setSaving(false)
    setOpen(false)
    setForm({ 施策名: '', カテゴリ: '学校小規模化対策', 緊急度: 'short', 担当部門: '', 期待効果: '', 根拠データ_実施前: '' })
    onRegister()
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 space-y-4">
        <h3 className="font-bold text-gray-800">施策を登録</h3>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              {['学校小規模化対策','ICT活用','移住促進','観光管理','人口対策','その他'].map(c => (
          ...
          <input
            type="text" placeholder="担当部門（例：教育委員会）"
            value={form.担当部門}
          ...
          />
          <textarea
            placeholder="根拠データ（実施前の数値：例「安房小学校の家庭Wi-Fi普及率65%、オンライン授業18回/年」）"
...
export function YakushimaPdcaPanel() {
  ...
  const loadPolicies = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/yakushima/pdca-tracking')
          ...
          {!evaluation ? (
            ...
            <>
              ...
              <div className="bg-green-50 border border-green-200 rounded-xl p-5">
                <p className="text-xs font-semibold text-green-600 mb-2">🤖 AI による PDCA 総括</p>
              ...
              {/* 現在のデータ */}
              {evaluation.dataChanges && (
                <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-2">
                  <p className="text-xs font-semibold text-gray-500 mb-3">📊 現在のデータ（5本のDB より）</p>
                  {Object.entries({
                    '学校': evaluation.dataChanges.schoolTrend,
                    'ICT':  evaluation.dataChanges.ictTrend,
                    '移住': evaluation.dataChanges.migrationTrend,
                    '観光': evaluation.dataChanges.tourismTrend,
                  }).map(([label, value]) => (
