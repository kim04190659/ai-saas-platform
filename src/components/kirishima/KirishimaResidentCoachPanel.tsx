'use client'
// =====================================================
//  src/components/kirishima/KirishimaResidentCoachPanel.tsx
//  霧島市 住民個人AIコーチ パネル — Sprint #49
//
//  屋久島町版（YakushimaResidentCoachPanel）と同じ UI 構造。
//  APIエンドポイントのみ /api/kirishima/resident-coach に変更。
//
...
function CategoryBadge({ category }: { category: string }) {
  const colors: Record<string, string> = {
    '廃棄物・環境':   'bg-green-100 text-green-700',
    '道路・インフラ': 'bg-orange-100 text-orange-700',
    '観光・騒音':     'bg-yellow-100 text-yellow-700',
    '高齢者支援':     'bg-purple-100 text-purple-700',
    '健康・医療':     'bg-pink-100 text-pink-700',
    '子育て・教育':   'bg-blue-100 text-blue-700',
...
export function KirishimaResidentCoachPanel() {
  ...
  const load = useCallback(async (keepSelectedId?: string) => {
    setLoading(true)
    try {
      const res  = await fetch('/api/kirishima/resident-coach')
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            相談履歴を分析し、住民ひとりひとりに合ったWell-Being改善提案を生成します（霧島市）
          </p>
        </div>
        <button
          onClick={handleCoachAll}
          disabled={allCoach || loading}
          className="px-4 py-2 bg-teal-600 hover:bg-teal-700 disabled:bg-gray-300 text-white text-sm font-medium rounded-lg transition"
        >
            ...
            <div className="flex-1 bg-white rounded-xl border border-gray-200 p-5 overflow-y-auto space-y-5">
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <button
                  ...
                  disabled={coaching}
                  className="shrink-0 px-4 py-2 bg-teal-600 hover:bg-teal-700 disabled:bg-gray-300 text-white text-sm font-medium rounded-lg transition"
                >
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-2xl font-bold text-teal-600">{selected.WBスコア ?? '—'} / 10</span>
                </div>
              ...
              <div>
                ...
                {selected.AIコーチングメッセージ && selected.AIコーチングメッセージ !== '（未実施）' ? (
                  <div className="text-sm text-gray-700 bg-teal-50 border border-teal-200 rounded-lg p-4 leading-relaxed whitespace-pre-wrap">
