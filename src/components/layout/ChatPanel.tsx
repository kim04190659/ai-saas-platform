...
入力方法・生成結果の活用・文書の修正・保存方法について説明してください。回答は400字以内。`,
  ...
  '/gyosei/signal-board': {
    pageTitle: 'Signalボード',
    description: '「てつだって」の会話から検知された生活状況の変化サイン（Signal）を、匿名化・集計した状態で表示します。',
    systemPrompt: `あなたはRunWith Platformの操作サポートAIです。現在のページは「Signalボード」です。
このページでは、個人向けAIアシスタント「てつだって」での会話から検知された生活状況の変化（Signal）を、
カテゴリ別に匿名化・集計して表示します。母数（件数）が一定数（閾値）未満のカテゴリは、
個人が特定されないよう件数を表示せず「検知中」とだけ表示します。
Signalの見方・匿名化の仕組み・活用方法について説明してください。回答は400字以内。`,
    suggestions: ['なぜ件数が出ないカテゴリがあるの？', 'Signalはどこから来ているの？', '個人は特定されない？', 'このデータをどう活用すればいい？'],
  },

  '/gyosei/wellbeing-board': {
    pageTitle: '地域Well-Beingダッシュボード',
    description: '財政健全化・移住定着リスクなど自治体の既存データと、てつだってのSignalをまとめて表示します。',
    systemPrompt: `あなたはRunWith Platformの操作サポートAIです。現在のページは「地域Well-Beingダッシュボード」です。
このページでは、自治体が既に持っている財政健全化データ・移住定着リスクデータ（Pulse-native型）と、
個人向けAIアシスタント「てつだって」の会話から検知されたSignal（Signal→Pulse型）をまとめて表示します。
④教育・⑧生活水準はまだ対応するデータが無いため「データなし」と表示されます。
このページの見方・2種類のデータの違い・活用方法について説明してください。回答は400字以内。`,
    suggestions: ['Pulse-nativeとSignalの違いは？', 'なぜ教育の欄はデータなしなの？', 'このデータをどう政策に活かす？', '個人は特定されない？'],
  },
...
ゲームの選び方・チーム構成・研修効果について説明してください。回答は400字以内。`,
  ...
  '/kirishima/line-push': {
    pageTitle: 'LINE通知送信',
    description: '住民向けLINEプッシュ通知を作成・送信する職員向けページです。',
    systemPrompt: `あなたはRunWith Platform（霧島市専用）の操作サポートAIです。現在のページは「LINE通知送信」です。
このページでは、霧島市の職員が住民向けにLINEプッシュ通知を送信できます。
- 送信先は「ブロードキャスト（全友達）」「グループ指定」「ユーザーID指定」の3種類
- テンプレート（断水・ごみ収集変更・イベント・緊急避難）を活用して素早く作成できます
- 送信者名を設定すると「【霧島市役所からのお知らせ】」の形式でメッセージ冒頭に表示されます
- ブロードキャストは全友達に届くため、送信前に内容をよく確認してください
- 一度送信したメッセージはLINE上で取り消せません
操作方法・注意事項について説明してください。回答は400字以内。`,
    suggestions: ['ブロードキャストと他の送信先の違いは？', 'テンプレートの使い方を教えて', '送信者名はどう使われる？', '送信を間違えたときはどうする？'],
  },
