...
> このリポジトリ（ai-saas-platform）は【結(YUI)】＝てつだって（個人）・Coarc（企業）・RunWith（自治体）を1つに統合した基盤の実体であり、GitHub上のリポジトリ名も近日中に「結（YUI）」を表す名称へ改名予定。旧称「RunWith Platform」は自治体レイヤーの呼称として引き続き使用する。
...
## 【結(YUI)】全体ガバナンスルール（Notion「プロジェクト進行ルール」より転記・2026-07-18）

> 詳細・最新版は常にNotion側を正とする。矛盾した場合はNotionを更新し、このセクションに反映すること。
...
- 想定フロー: カードゲームで体験 → 行政Oス（`/gyosei`）で地域診断 → RunWith（`/runwith`）でIT基盤実装 → Notionにナレッジ保存（`/api/notion-save`, `/api/notion`）。新機能はこのフローのどこに位置づくかを意識する。
